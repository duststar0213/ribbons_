...
public class ribbons_ extends PApplet {
...
public void drawFlower(float x, float y,float radius){
 ...
 // fill(0,map(y,height,20,0.2,50),map(x,width,20,0.2,200),40);    //blue
   int col = color(round(map(x,0.1f,width/10,10,255)), angle, 255,75); //rainbow color code
   fill(col);
 diameter = 2;
 float radius1 = 1 +  (cos(angle/mouseX) * diameter/4) + sin(angle/mouseX);
float radius2 = diameter- diameter*sin(angle/mouseX) + sin(angle/mouseX)*sqrt(abs(cos(angle/mouseY))/(sin(angle/mouseX)+1.4f));
...
public void star(float x, float y, float radius1, float radius2, int npoints) {
  ...
  public void settings() {  size(1200,1200,P2D);  smooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "ribbons_" };
    if (passedArgs != null) {
